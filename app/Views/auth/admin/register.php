            ...
            <form action="<?= base_url('admin/product/save'); ?>" method="post">
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" username="username" id="username" class="<?= ($validation && $validation->hasError('username')) ? 'input-error' : 'input-admin' ?>" <?= ($validation && $validation->hasError('username')) ? 'autofocus' : '' ?> value="<?= old('username'); ?>" placeholder="johndoe" />
