    ...
    <div class="p-4 sm:ml-64">
        <div class="h-full bg-white rounded-lg shadow-sm p-4 mt-16 md:mt-14">
            <div class="flex items-center justify-between">
                ...
                <p class="text-xs text-gray-500 tracking-wide">14 April 2024, pukul 13.05 WIB</p>
            </div>
        </div>
        <div class="h-full bg-white rounded-lg p-4 mt-4">
            <!-- Order start -->
                ...
                <?php foreach ($cart as $item) : ?>
                    <div class="space-y-2 mb-2">
                        <div class="flex justify-between items-center">
                            <p class="w-2/4 text-xs md:text-sm text-gray-500 text-left tracking-wide"><?= $item['product_name']; ?> &times;<?= $item['quantity']; ?></p>
                            <p class="w-1/4 text-xs md:text-sm text-gray-500 text-end tracking-wide"><?= 'Rp. ' . number_format($item['total'], 0, ',', '.'); ?></p>
                        </div>
                    </div>
                <?php endforeach; ?>
                <div class="flex justify-between items-center border-t pt-2">
                    <p class="text-base md:text-lg font-semibold tracking-wide">Total</p>
                    <p class="text-base md:text-lg font-semibold tracking-wide"><?= 'Rp. ' . number_format($total_price, 0, ',', '.'); ?></p>
                </div>
            </div>
            <!-- Order end -->
            <button id="pay-button" type="button" class="btn-primary w-full flex items-center justify-center space-x-2">
