...
<div class="min-h-screen">
    <div class="p-4 sm:ml-64">
        ...
        <div class="h-full bg-white rounded-lg shadow-sm p-4 mt-3 md:mt-4">
            <form action="<?= base_url("admin/$admin[id]update"); ?>" method="post">
                <?= csrf_field(); ?>
                <div class="space-y-5">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-x-0 gap-y-4 md:gap-x-4">
                        ...
                        <div class="col-span-2 space-y-5">
                            <div>
                                <label for="fullname" class="text-sm font-medium text-myBlack tracking-wide">Nama Depan <span class="text-red-500">*</span></label>
                                <input type="text" name="fullname" id="fullname" class="<?= ($validation && $validation->hasError('fullname')) ? 'input-error' : 'input-admin' ?>" <?= ($validation && $validation->hasError('fullname')) ? 'autofocus' : '' ?> placeholder="John" value="<?= old('fullname', $admin['fullname']) ?>" />
                            ...
                            <div>
                                ...
                                <input type="text" name="username" id="username" class="<?= ($validation && $validation->hasError('username')) ? 'input-error' : 'input-admin' ?>" <?= ($validation && $validation->hasError('username')) ? 'autofocus' : '' ?> value="<?= old('username', $admin['username']); ?>" placeholder="johndoe" />
                            ...
                            <div>
                                ...
                                <input type="email" name="email" id="email" class="<?= ($validation && $validation->hasError('email')) ? 'input-error' : 'input-admin' ?>" <?= ($validation && $validation->hasError('email')) ? 'autofocus' : '' ?> placeholder="someone@example.com" value="<?= old('email', $admin['email']) ?>" />
                            ...
                            <div>
                                ...
                                <input type="text" name="phone_number" id="phone_number" class="<?= ($validation && $validation->hasError('phone_number')) ? 'input-error' : 'input-admin' ?>" <?= ($validation && $validation->hasError('phone_number')) ? 'autofocus' : '' ?> placeholder="+62" value="<?= old('phone_number', $admin['phone_number']) ?>" />
